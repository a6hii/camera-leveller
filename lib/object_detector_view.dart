...
import 'package:obj_detection_poc/object_detector_painter.dart';
...
class _ObjectDetectorView extends State<ObjectDetectorView> {
  ...
  void _initializeDetector() async {
    ...
    if (_option == 0) {
      ...
      final modelPath = await getAssetPath('assets/tflite/$option');
  ...
  Future<void> _processImage(InputImage inputImage) async {
    ...
    print(
        "inputImage.metadata?.size: ${objects?.map((e) => e.labels.map((e) => e.text))}\n"
        "inputImage.metadata?.size11: ${inputImage.metadata?.size}");
    if (inputImage.metadata?.size != null &&
      ...
      for (final object in objects!) {
        text +=
            'Object:  trackingId: ${object.trackingId} - ${object.labels.map((e) => e.text).toList().join(', ')}\n\n';
      }
      _text = text;
